...
/*ZTE_MODIFY by dll for headset detect stable 2016/01/22 start*/
#define ZTE_MASK_HEADSET_DETECT_STABLE 
/*ZTE_MODIFY by dll for headset detect stable 2014/01/22 end*/
...
/*ZTE_MODIFY by liyang for fastmmi 2014/07/07 end*/
	if (!insertion) {
		...
		pr_err("%s: Reporting removal %d(%x)\n", __func__,
			 jack_type, mbhc->hph_status);//zte liyang use to print the headset information
		wcd_mbhc_jack_report(mbhc, &mbhc->headset_jack,
				mbhc->hph_status, WCD_MBHC_JACK_MASK);
...
exit:
   ...
   /*ZTE_MODIFY by lijilou  for solving the headset remove ,the headset icon diappear in 4-12 seconds start*/
   #if 0
	if (mbhc->mbhc_cfg->detect_extn_cable &&
	    ((plug_type == MBHC_PLUG_TYPE_HEADPHONE) ||
	     (plug_type == MBHC_PLUG_TYPE_HEADSET)) &&
    ...
    #endif
  /*ZTE_MODIFY by lijilou  for solving the headset remove ,the headset icon diappear in 4-12 seconds end*/
	if (mbhc->mbhc_cb->set_cap_mode)
