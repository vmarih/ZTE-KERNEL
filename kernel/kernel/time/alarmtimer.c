...
#define ALARM_DELTA 120  // ZTE_MODIFY for poweroff alarm by dingli10091962
...
void set_power_on_alarm(void)
{
	...
	if (next) {
        ...
        // ZTE_MODIFY start for poweroff alarm by dingli10091962
        ...
        // ZTE_MODIFY end for poweroff alarm by dingli10091962
	}
...
static enum hrtimer_restart alarmtimer_fired(struct hrtimer *timer)
{
	...
	if (alarm->type == ALARM_POWEROFF_REALTIME) {
		...
		if (strcmp(boot_mode, "charger") == 0) { 
			...
			//printk("boot_time.tv_sec = %lld, boot_time.tv_sec = %ld\n",boot_time.tv_sec,boot_time.tv_nsec);	
			kt_boottime = timespec_to_ktime(current_time);
		        printk("kt_boottime = %lld\n",kt_boottime.tv64);	
			kt_alarm = ktime_add(kt_boottime, alarm_boottime_delta);			
			alarm_start(alarm, kt_alarm);		
			}   
