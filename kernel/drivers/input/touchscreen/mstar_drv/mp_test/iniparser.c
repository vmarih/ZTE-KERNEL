    ...
    char * value)
{
    ...
    // printk("================= Parsing start ============= \n");
    // printk("line = %s \n", line);
    // printk("line[0] = %c , line[len-1] =%c, \n", line[0], line[len-1]);
    // printk("length = %d \n", len);
    // printk("key = %p, section = %p , value = %p \n", &key, &section, &value);
    //printk("delim:  %c \n", delim[0]);
...
common_case:
    ...
    //printk("usual case: key = %s , value = %s \n", key, value);
    strstrip_t(key);
...
//    loff_t pos = 0;
    ...
    if ((in=file_open(ininame, O_RDONLY, 0, &fsize))==NULL) {
        printk("iniparser: cannot open %s\n", ininame);
        return NULL ;
    }
...
//    while (fgets(line+last, ASCIILINESZ-last, in)!=NULL) {
    while (kgets(line+last, ASCIILINESZ-last, in)!=NULL) {
        ...
        if (line[len]!='\n' && line[len] != 255 && line[len] != 0 && line[len] != 30) {
           printk("iniparse: input line too long in %s (%d) \n", ininame, lineno);
           printk("lineno = %d, %d\n", lineno, line[len]);
           dictionary_del(dict);
           //fclose(in);
           file_close(in);
	 ...
	 //	break;
        ...
        switch (iniparser_line(line, section, key, val)) {
            ...
            //printk("%s:  section = %s , key = %s , val = %s\n", __func__, section, key, val);
            errs = dictionary_set(dict, tmp, val) ;
            break ;

            case LINE_ERROR:
            printk("iniparser: syntax error in %s (%d): \n", ininame, lineno);
            printk("-> %s\n", line);
            errs++ ;
            break;
        ...
        last=0;
        if (errs<0) {
            printk("iniparser: memory allocation failure\n");
            break ;
        }
    }
    if (errs) {
