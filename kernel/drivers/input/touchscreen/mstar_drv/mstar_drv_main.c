...
extern struct mutex g_Mutex_patch;
...
#ifdef CONFIG_TPD_MSTAR_TEST
static int _mstar_test_init(void);
#endif
...
ssize_t DrvMainProcfsMpTestCustomisedWrite(struct file *pFile, const char __user *pBuffer, size_t nCount, loff_t *pPos)  
{
    char *pValid = NULL;
...
{    
    ...
    if (pBuffer != NULL)
    {
        ...
        if (nFeature == FEATURE_GESTURE_WAKEUP_MODE) // Not support dynamically on/off gesture wakeup mode by adb command yet.
        {
            _gFeatureSupportStatus = IS_GESTURE_WAKEUP_ENABLED;
        }
        else if (nFeature == FEATURE_GESTURE_DEBUG_MODE) // Not support dynamically on/off gesture debug mode by adb command yet.
        {
            _gFeatureSupportStatus = IS_GESTURE_DEBUG_MODE_ENABLED;
        }
        else if (nFeature == FEATURE_GESTURE_INFORMATION_MODE) // Not support dynamically on/off gesture information mode by adb command yet.
        {
...
//	static int node_opened = 0;
	...
	sprintf(filepath, "%s%s", g_str_ini_file_path, g_str_ini_filename);
...
//			MSTAR_TEST_DBG("command:0x%ld,  open node before start test.", command);
//		}
...
static int tpd_test_save_file_path_store(struct tpd_classdev_t *cdev, const char *buf)
{
	...
	MSTAR_TEST_DBG("save file path:%s.", g_str_save_file_path);
...
static int tpd_test_channel_show(struct tpd_classdev_t *cdev, char *buf)
{
	...
	tpd_test_get_channel_setting(&buffer[0]);
	g_mstar_test_buffer.i_txNum = buffer[0];
	g_mstar_test_buffer.i_rxNum = buffer[1];
...
static int _mstar_test_init(void)
{
	...
	strncpy(g_str_save_file_path, MSTAR_INI_FILE_PATH, 256);
	strncpy(g_str_ini_file_path, MSTAR_INI_FILE_PATH, 256);
