        ...
        if (bIsCorrectFormat)
        {
            ...
            switch (nWakeupMode)
            {
                case 0x58:
                    ...
                    printk( "Light up screen by DOUBLE_CLICK gesture wakeup %d.\n",GESTURE_WAKEUP_MODE_DOUBLE_CLICK_FLAG);
...
*/
                case 0x62:
                    ...
                    printk("Light up screen by LEFT_DIRECT gesture wakeup %d.\n",KEY_GESTURE_LEFT);
...
//                    input_report_key(g_InputDevice, KEY_LEFT, 0);
                ...
                case 0x63:
                    ...
                    printk("Light up screen by RIGHT_DIRECT gesture wakeup %d.\n",KEY_GESTURE_RIGHT);
...
static int tpd_enable_wakegesture(struct tpd_classdev_t *cdev,int enable)
{
	...
	g_GestureWakeup = enable;

     DBG(&g_I2cClient->dev, "g_GestureWakeup = %d\n", g_GestureWakeup);
...
static int tpd_init_tpinfo(struct tpd_classdev_t *cdev)
{
	...
	strcpy(cdev->ic_tpinfo.tp_name, "Mstar");
	cdev->ic_tpinfo.chip_model_id = 6;
	
	cdev->ic_tpinfo.chip_part_id= 0x307;
	cdev->ic_tpinfo.module_id= nMajor;
	cdev->ic_tpinfo.chip_ver = 0;
	cdev->ic_tpinfo.firmware_ver= nMinor;
	cdev->ic_tpinfo.i2c_type = 0;
	cdev->ic_tpinfo.i2c_addr = 0x26;
	return 0;
}

int	tpd_register_fw_class(struct i2c_client *client)
{

	tpd_init_tpinfo(&tpd_fw_cdev);
	tpd_fw_cdev.wake_gesture_mode = tpd_enable_wakegesture;
