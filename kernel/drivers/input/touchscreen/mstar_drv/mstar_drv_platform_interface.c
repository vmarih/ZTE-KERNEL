    ...
    if (pEventData && pEventData->data && nEvent == FB_EVENT_BLANK)
    {
...
#endif //CONFIG_ENABLE_GESTURE_DEBUG_MODE
#ifdef CONFIG_CREATE_TPD_SYS_INTERFACE
                if (g_GestureWakeupFlag == 1 && 1 == g_GestureWakeup)
#else
        ...
        else if (*pBlank == FB_BLANK_POWERDOWN)
        {
...
#endif //CONFIG_ENABLE_HOTKNOT
            {
                ...
                if( (g_GestureWakeupMode[0] != 0x00000000 || g_GestureWakeupMode[1] != 0x00000000)&&1 == g_GestureWakeup)
#else
...
extern int tpd_register_fw_class(struct i2c_client *client);
...
s32 /*__devinit*/ MsDrvInterfaceTouchDeviceProbe(struct i2c_client *pClient, const struct i2c_device_id *pDeviceId)
{
 ...
 if(!tpd_power_already_on)
 {
    ...
    DBG(&g_I2cClient->dev, "*** %s() *** already power on\n", __func__);
    return -ENODEV;
