...
/*----------------------------------- other define-----------------------------------*/
char save_result_dir[250] = "/data/data/";
...
extern struct gtx_test_buffer g_gtx_test_buffer;
//struct gtx_test_buffer g_gtx_test_buffer1;
...
int gtx_save_string_to_buffer(struct gtx_test_buffer *stp_test, char * tmp_buffer, int length)
{
	//TPD_DMESG("length:%d", length);
	
	if(NULL == stp_test->procedure_buffer || (stp_test->procedure_length + length) > TEST_RESULT_LENGTH) {
		DEBUG("warning:buffer is null or buffer overflow, return");
		return -1;
	}
...
int gtx_save_test_result(struct gtx_test_buffer *stp_test)
{
	int i_len = 0;
	int result = stp_test->test_result;
			...
			//unsigned short result_addr;
			//tpd_test_get_channel_setting(&buffer[0]);
			//i2c_read_data(0x8802, short_status, 3);
			stp_test->i_txNum = 12 ;
			stp_test->i_rxNum = 20;
			//DEBUG("gtx_save_test_result Tx&Tx:%d,Rx&Rx:%d,Tx&Rx:%d", short_status[0], short_status[1], short_status[2]);
			
			stp_test->result_buffer = (char*)malloc(TEST_RESULT_LENGTH);
			stp_test->result_length = 6;
		}
	...
	gtx_save_string_to_buffer(stp_test, stp_test->temp_buffer, i_len);
...
//  if ((check_types & _TEST_RESULT_SAVE) != 0)
	{
	    //zz
            g_gtx_test_buffer.test_result = test_error_code;  
		gtx_save_test_result(&g_gtx_test_buffer);
