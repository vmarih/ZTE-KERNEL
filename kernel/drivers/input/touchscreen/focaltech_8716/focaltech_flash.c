...
unsigned char CTPM_FW_TIANMA[] =
{
#include FTS_UPGRADE_FW_APP_TIANMA
};
...
u32 fts_getsize(u8 fw_type)
{
    ...
    if (fw_type == FW_SIZE)
		if(ft_vendor_id == 0x8d)
		{
            fw_len = sizeof(CTPM_FW_TIANMA);
			printk("fts,get vendor_id,%d\n",fw_len);
		}
		else
		{
			fw_len = 0;
			//printk("fts,can not get vendor_id\n");
		}
#if FTS_CHIP_IDC
