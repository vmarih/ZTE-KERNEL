...
unsigned char FT8736_TestItem_CbTest(bool * bTestResult)
{
	...
	for (iCol = 0; iCol < ReadKeyLen /*g_stSCapConfEx.KeyNumTotal */ ; ++iCol) {
		if (ucBits != 0) {
			m_CBData[g_stSCapConfEx.ChannelXNum][iCol / 2] =
				(short)((m_ucTempData[g_stSCapConfEx.ChannelXNum * g_stSCapConfEx.ChannelYNum + iCol] & 0x01) << 8) + m_ucTempData[g_stSCapConfEx.ChannelXNum * g_stSCapConfEx.ChannelYNum + iCol + 1];
			iCol++;
		}
		else {
	...
	for (iRow = 0; iRow < g_stSCapConfEx.ChannelXNum; iRow++) {
		for (iCol = 0; iCol < g_stSCapConfEx.ChannelYNum; iCol++) {
			...
			if (iValue < iMinValue || iValue > iMaxValue) {
				btmpresult = false;
				FTS_TEST_ERROR("CB test failure. Node=(%d,  %d), Get_value=%d,  Set_Range=(%d, %d) ", iRow + 1, iCol + 1, iValue, iMinValue, iMaxValue);
			}
		}
	}
	...
	if (bIncludeKey) {
		...
		for (iCol = 0; iCol < g_stSCapConfEx.KeyNumTotal; iCol++) {
			...
			if (iValue < iMinValue || iValue > iMaxValue) {
				btmpresult = false;
				FTS_TEST_ERROR("rawdata test failure. Node=(%d,  %d), Get_value=%d,  Set_Range=(%d, %d) ", iRow + 1, iCol + 1, iValue, iMinValue, iMaxValue);
			}
		}
		...
		if (ReCode == ERROR_CODE_OK) {
			if (0 < rBuffer[0] && rBuffer[0] < 80) {
				...
				if (g_stSCapConfEx.ChannelXNum > g_ScreenSetParam.iUsedMaxTxNum) {
					FTS_TEST_ERROR("Failed to get Channel X number, Get num = %d, UsedMaxNum = %d", g_stSCapConfEx.ChannelXNum, g_ScreenSetParam.iUsedMaxTxNum);
		...
		if (ReCode == ERROR_CODE_OK) {
			if (0 < rBuffer[0] && rBuffer[0] < 80) {
				...
				if (g_stSCapConfEx.ChannelYNum > g_ScreenSetParam.iUsedMaxRxNum) {
					FTS_TEST_ERROR("Failed to get Channel Y number, Get num = %d, UsedMaxNum = %d", g_stSCapConfEx.ChannelYNum, g_ScreenSetParam.iUsedMaxRxNum);
