...
int save_string_to_buffer(struct fts_test_buffer *stp_test, char * tmp_buffer, int length)
{
	//TPD_DMESG("length:%d", length);
...
{
	save_string_to_buffer(&g_fts_test_buffer, str, strlen(str));

	return 0;
}

int save_test_result(struct fts_test_buffer *stp_test)
{
	int i_len = 0;
...
int save_ito_array_data_to_file(struct fts_test_buffer *stp_test, char *title_info, void *data, int length, int size, int i_row, int i_col)
{
	int i_len = 0;
	int i = 0, j = 0;
	int i_value = 0;
...
{
	return save_ito_array_data_to_file(&g_fts_test_buffer, title_info, data, length, node_byte, g_fts_test_buffer.i_txNum, g_fts_test_buffer.i_rxNum);
}
...
static int fts_test_entry(char *ini_file_name)
{
...
#ifdef CONFIG_TPD_FOCAL_TEST
	{
		...
		save_test_result(&g_fts_test_buffer);
