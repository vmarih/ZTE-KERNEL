...
int tpd_focal_get_version_etc(struct tpd_classdev_t *cdev, int * chipid_in_chip, int *fwver_in_chip, int *module_id_in_chip)
{
	int i_ret = 0;
	struct fts_ts_data *data = (struct fts_ts_data*) cdev->private;

	if (fts_updatefun_curr.tpd_get_version_etc)
		i_ret = fts_updatefun_curr.tpd_get_version_etc(data->client, chipid_in_chip, fwver_in_chip, module_id_in_chip);

	return i_ret;
}

int tpd_focal_firmware_upgrade(struct tpd_classdev_t *cdev, unsigned char * pbt_buf, unsigned int size, unsigned int force_upg)
{
	int i_ret = 0;
	struct fts_ts_data *data = (struct fts_ts_data*) cdev->private;

	if (fts_updatefun_curr.tpd_firmware_upgrade)
		i_ret = fts_updatefun_curr.tpd_firmware_upgrade(data->client, pbt_buf, size, force_upg);
