				...
				for (i = 0; i < data->pdata->key_number; i++) {
					if (event->au16_x[0] > (data->pdata->key_x_coords[i] - FTS_KEY_WIDTH) && event->au16_x[0] < (data->pdata->key_x_coords[i] + FTS_KEY_WIDTH)) {
						...
						if (event->au8_touch_event[i] == 0 || event->au8_touch_event[i] == 2) {
							...
							FTS_INFO("tpd Key%d(%d, %d) DOWN!", i, event->au16_x[0], event->au16_y[0]);
						}
						else {
							input_report_key(data->input_dev, data->pdata->keys[i], 0);
							FTS_INFO("tpd Key%d(%d, %d) Up!", i, event->au16_x[0], event->au16_y[0]);
						}
						break;
					}
				}
...
#ifdef CONFIG_CREATE_TPD_SYS_INTERFACE
			debug_point_print(event->au8_finger_id[i], event->au16_x[i], event->au16_y[i], event->area[i], 1);
#endif
...
#endif
#ifdef CONFIG_CREATE_TPD_SYS_INTERFACE
			debug_point_print(event->au8_finger_id[i], event->au16_x[i], event->au16_y[i], event->area[i], 0);
#endif
			data->touchs &= ~BIT(event->au8_finger_id[i]);
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
	...
	FTS_INFO("get resume_in_workqueue= %d\n", pdata->resume_in_workqueue);
...
{
	struct fts_ts_data *fts_data = container_of(work, struct fts_ts_data, fb_notify_work);
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	FTS_INFO("tpd: %s define CHIP TYPE:0x%08x IC:0x%08x, IDC:0x%08x\n", __func__, FTS_CHIP_TYPE, IC_SERIALS, FTS_CHIP_IDC);
	...
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {//gaoyuan
		printk("TPD I2C not supported");
		FTS_FUNC_EXIT();
		return -ENODEV;
	}
	...
	err = request_threaded_irq(client->irq, NULL, fts_ts_interrupt, IRQF_ONESHOT | IRQF_TRIGGER_FALLING, client->dev.driver->name, data);
	if (err) {
...
#if FTS_GESTURE_EN
	if(data->tpd_enable_wakeup_gesture) {
