...
extern int save_failed_node(char * str);
extern int save_test_procedure(char * str);
extern int save_rawdata_to_file(char *title_info, void *data, int length, int node_byte);

char temp_buffer[256];
int g_failed_node[TX_NUM_MAX][RX_NUM_MAX];

#define tpd_test_save_procedure(fmt, ...) do { snprintf(temp_buffer, 256, fmt, ##__VA_ARGS__); save_test_procedure(temp_buffer);}while(0);
#define tpd_test_save_failed_node(fmt, ...) do { snprintf(temp_buffer, 256, fmt, ##__VA_ARGS__); save_failed_node(temp_buffer);}while(0);
#endif
...
#ifdef CONFIG_TPD_FOCAL_TEST
static void tpd_save_short_array_to_file(short iData[TX_NUM_MAX][RX_NUM_MAX], unsigned char i_row, unsigned char i_col)
{
	int i_len = 0;
	int i = 0, j = 0;
	char buffer[64];
	...
	for(i = 0; i < i_row; i++) {
		for(j = 0; j < i_col; j++) {
			i_value = iData[i][j];
			if(j == (i_col -1)) 
				i_len= sprintf(buffer,"%d, \n",  i_value);	
			else
				i_len= sprintf(buffer,"%d, ", i_value);	

			save_test_procedure(buffer);
			
			if (g_stCfg_MCap_DetailThreshold.InvalidNode[i_row][i_col] == 0)
	...
	i_len= sprintf(buffer,"Data min:%d max:%d average:%ld\n", i_rawdata_min, i_rawdata_max, i_rawdata_average / i_rawdata_count);
	save_test_procedure(buffer);
}

static void tpd_save_array_to_file(int iData[TX_NUM_MAX][RX_NUM_MAX], unsigned char i_row, unsigned char i_col)
{
	int i_len = 0;
	int i = 0, j = 0;
	char buffer[128];
	...
	if(i_row >= TX_NUM_MAX || i_col >= RX_NUM_MAX) {
		return;
	}
	...
	for(i = 0; i < i_row; i++) {
		for(j = 0; j < i_col; j++) {
			...
			if (g_stCfg_MCap_DetailThreshold.InvalidNode[i][j] == 0) {
				//tpd_test_save_procedure("invalidNode (%d %d)", i, j);
				continue;	//Invalid Node
			}
...
static void tpd_save_SC_array_to_file(int iData[TX_NUM_MAX][RX_NUM_MAX], int index, unsigned char i_row, unsigned char i_col)
{
	int i_len = 0;
	int i = 0, j = 0;
	char buffer[64];
	...
	for(i = 0 + index; i < i_row + index; i++) {
		for(j = 0; j < i_col; j++) {
			...
			if (g_stCfg_MCap_DetailThreshold.InvalidNode_SC[i][j] == 0) {
				continue;	//Invalid Node
			}
...
int set_failed_node(int tx, int rx)
{
	g_failed_node[tx][rx] = 1;
	return 0;
}

int init_failed_node_value(void)
{
	int i = 0, j = 0;
...
int fill_failed_node_value(void)
{
	int i = 0, j = 0;

	for(i = 0; i < TX_NUM_MAX; i++) {
		for(j = 0; j < RX_NUM_MAX; j++) {
			if(1 == g_failed_node[i][j]) {
				tpd_test_save_failed_node(",%d,%d", i, j);
			}
		}
	}
	return 0;
...
unsigned char FT5X46_TestItem_RawDataTest(bool * bTestResult)
{
...
#ifdef CONFIG_TPD_FOCAL_TEST
	tpd_test_save_procedure("RawData test m_bV3TP %d, Normalize %s\n", m_bV3TP, g_ScreenSetParam.isNormalize == Auto_Normalize ? "Auto_Normalize" : "Overall_Normalize");
#endif
    ...
    if (g_ScreenSetParam.isNormalize == Auto_Normalize)
    {
        ...
        ShowRawData();
#ifdef CONFIG_TPD_FOCAL_TEST
		tpd_test_save_procedure("Auto_Normalize Set Frequecy High RawData test  Tx %d Rx %d Higth Min\n", g_ScreenSetParam.iTxNum, g_ScreenSetParam.iRxNum);
		tpd_save_array_to_file(g_stCfg_MCap_DetailThreshold.RawDataTest_High_Min, g_ScreenSetParam.iTxNum, g_ScreenSetParam.iRxNum);
		
		tpd_test_save_procedure("RawData test Tx %d Rx %d Higth Max\n", g_ScreenSetParam.iTxNum, g_ScreenSetParam.iRxNum);
		tpd_save_array_to_file(g_stCfg_MCap_DetailThreshold.RawDataTest_High_Max, g_ScreenSetParam.iTxNum, g_ScreenSetParam.iRxNum);
		
		tpd_test_save_procedure("RawData test, Tx %d Rx %d RawData\n", g_ScreenSetParam.iTxNum, g_ScreenSetParam.iRxNum);
		tpd_save_array_to_file(m_RawData, g_ScreenSetParam.iTxNum, g_ScreenSetParam.iRxNum);
#endif
        ...
        for (iRow = 0; iRow<g_ScreenSetParam.iTxNum; iRow++)
        {
            for (iCol = 0; iCol < g_ScreenSetParam.iRxNum; iCol++)
            {
                ...
                if (iValue < RawDataMin){
                    btmpresult = false;
					set_failed_node(iRow, iCol);
					g_int_tptest_result |= TEST_BEYOND_MIN_LIMIT;
					tpd_test_save_procedure("rawdata test failure  Node=(%d %d) Get_value=%d  Set_Range=(%d - %d)\n", iRow, iCol, iValue, RawDataMin, RawDataMax);
					FTS_TEST_ERROR("rawdata test failure. Node=(%d,  %d), Get_value=%d,  Set_Range=(%d, %d) ", iRow + 1, iCol + 1, iValue, RawDataMin, RawDataMax);
            }
				if (iValue > RawDataMax) {
					btmpresult = false;
					set_failed_node(iRow, iCol);
					g_int_tptest_result |= TEST_BEYOND_MAX_LIMIT;
					tpd_test_save_procedure("rawdata test failure  Node=(%d %d) Get_value=%d  Set_Range=(%d - %d)\n", iRow, iCol, iValue, RawDataMin, RawDataMax);
					FTS_TEST_ERROR("rawdata test failure. Node=(%d,  %d), Get_value=%d,  Set_Range=(%d, %d) ", iRow + 1, iCol + 1, iValue, RawDataMin, RawDataMax);
        }
    ...
    else
    {
        ...
        if (g_stCfg_FT5X46_BasicThreshold.RawDataTest_SetLowFreq)
        {
            ...
            */
            ...
            ShowRawData();
#ifdef CONFIG_TPD_FOCAL_TEST
			tpd_test_save_procedure("Set Frequecy Low RawData test Tx %d Rx %d Higth Min\n", g_ScreenSetParam.iTxNum, g_ScreenSetParam.iRxNum);
			tpd_save_array_to_file(g_stCfg_MCap_DetailThreshold.RawDataTest_Low_Min, g_ScreenSetParam.iTxNum, g_ScreenSetParam.iRxNum);
			
			tpd_test_save_procedure("RawData test Tx %d Rx %d Higth Max\n", g_ScreenSetParam.iTxNum, g_ScreenSetParam.iRxNum);
			tpd_save_array_to_file(g_stCfg_MCap_DetailThreshold.RawDataTest_Low_Max, g_ScreenSetParam.iTxNum, g_ScreenSetParam.iRxNum);
        ...
        if ( g_stCfg_FT5X46_BasicThreshold.RawDataTest_SetHighFreq )
        {
            ...
            ShowRawData();
#ifdef CONFIG_TPD_FOCAL_TEST
			tpd_test_save_procedure("Set Frequecy High RawData test Tx %d Rx %d Higth Min\n", g_ScreenSetParam.iTxNum, g_ScreenSetParam.iRxNum);
			tpd_save_array_to_file(g_stCfg_MCap_DetailThreshold.RawDataTest_High_Min, g_ScreenSetParam.iTxNum, g_ScreenSetParam.iRxNum);
...
#ifdef CONFIG_TPD_FOCAL_TEST
	tpd_test_save_procedure("Scap RawData test\n");
#endif
    //-------1.Preparatory work
    //in Factory Mode
    ReCode = EnterFactory();
    ...
    if (g_stCfg_FT5X46_BasicThreshold.SCapRawDataTest_SetWaterproof_ON && bFlag )
    {
...
#ifdef CONFIG_TPD_FOCAL_TEST
		tpd_test_save_procedure("SCapRawDataTest_ON_Min  Tx %d Rx %d on Min\n", g_ScreenSetParam.iTxNum, g_ScreenSetParam.iRxNum);
		tpd_save_array_to_file(g_stCfg_MCap_DetailThreshold.SCapRawDataTest_ON_Min, 2, g_ScreenSetParam.iRxNum);
		
		tpd_test_save_procedure("SCapRawDataTest_ON_Max Tx %d Rx %d on Max\n", g_ScreenSetParam.iTxNum, g_ScreenSetParam.iRxNum);
		tpd_save_array_to_file(g_stCfg_MCap_DetailThreshold.SCapRawDataTest_ON_Max, 2, g_ScreenSetParam.iRxNum);
		
		tpd_test_save_procedure("Judge Rx in Waterproof-ON Tx %d Rx %d RawData\n", g_ScreenSetParam.iTxNum, g_ScreenSetParam.iRxNum);
		tpd_save_SC_array_to_file(m_RawData, g_ScreenSetParam.iTxNum + 0, 2, g_ScreenSetParam.iRxNum);
#endif
        for ( i = 0; bFlag && i < g_ScreenSetParam.iRxNum; i++ )
        {
			...
			if (Value > RawDataMax || Value < RawDataMin) {
				btmpresult = false;
				g_int_tptest_result |= TEST_BEYOND_MIN_LIMIT;
				set_failed_node(0, i + 1);
				tpd_test_save_procedure("Failed. Num = %d, Value = %d, range = (%d, %d):", i + 1, Value, RawDataMin, RawDataMax);
...
#ifdef CONFIG_TPD_FOCAL_TEST
		tpd_test_save_procedure("Judge Tx in Waterproof-ON Tx %d Rx %d RawData\n", g_ScreenSetParam.iTxNum, g_ScreenSetParam.iRxNum);
		tpd_save_SC_array_to_file(m_RawData, g_ScreenSetParam.iTxNum + 0, 2, g_ScreenSetParam.iRxNum);
#endif
...
#ifdef CONFIG_TPD_FOCAL_TEST
		tpd_test_save_procedure("SCapRawDataTest_OFF_Min  Tx %d Rx %d on Min\n", g_ScreenSetParam.iTxNum, g_ScreenSetParam.iRxNum);
		tpd_save_array_to_file(g_stCfg_MCap_DetailThreshold.SCapRawDataTest_OFF_Min, 2, g_ScreenSetParam.iRxNum);
		
		tpd_test_save_procedure("SCapRawDataTest_OFF_Max Tx %d Rx %d on Max\n", g_ScreenSetParam.iTxNum, g_ScreenSetParam.iRxNum);
		tpd_save_array_to_file(g_stCfg_MCap_DetailThreshold.SCapRawDataTest_OFF_Max, 2, g_ScreenSetParam.iRxNum);
		
		tpd_test_save_procedure("Judge Rx in Waterproof-OFF Tx %d Rx %d RawData\n", g_ScreenSetParam.iTxNum, g_ScreenSetParam.iRxNum);
		tpd_save_SC_array_to_file(m_RawData, g_ScreenSetParam.iTxNum + 0, 2, g_ScreenSetParam.iRxNum);
#endif
        for (i = 0; bFlag && i < g_ScreenSetParam.iRxNum; i++)
        {
...
#ifdef CONFIG_TPD_FOCAL_TEST
		tpd_test_save_procedure("Judge Tx in Waterproof-OFF Tx %d Rx %d RawData\n", g_ScreenSetParam.iTxNum, g_ScreenSetParam.iRxNum);
		tpd_save_SC_array_to_file(m_RawData, g_ScreenSetParam.iTxNum + 2, 2, g_ScreenSetParam.iRxNum);
#endif
        for (i = 0; bFlag && i < g_ScreenSetParam.iTxNum; i++)
        {
            if ( g_stCfg_MCap_DetailThreshold.InvalidNode_SC[1][i] == 0 )      continue;
...
#ifdef CONFIG_TPD_FOCAL_TEST
	tpd_test_save_procedure("Scap CB test\n");
#endif
...
#ifdef CONFIG_TPD_FOCAL_TEST
		tpd_test_save_procedure("SCapCbTest_ON_Min  Tx %d Rx %d on Min\n", g_ScreenSetParam.iTxNum, g_ScreenSetParam.iRxNum);
		tpd_save_short_array_to_file(g_stCfg_MCap_DetailThreshold.SCapCbTest_ON_Min, 2, g_ScreenSetParam.iRxNum);
		
		tpd_test_save_procedure("SCapCbTest_ON_Max Tx %d Rx %d on Max\n", g_ScreenSetParam.iTxNum, g_ScreenSetParam.iRxNum);
		tpd_save_short_array_to_file(g_stCfg_MCap_DetailThreshold.SCapCbTest_ON_Max, 2, g_ScreenSetParam.iRxNum);
		
		tpd_test_save_procedure("CB_Rx Tx %d Rx %d RawData\n", g_ScreenSetParam.iTxNum, g_ScreenSetParam.iRxNum);
		tpd_save_SC_array_to_file(m_RawData, g_ScreenSetParam.iTxNum + 0, 2, g_ScreenSetParam.iRxNum);
#endif
        for ( i = 0; bFlag && i < g_ScreenSetParam.iRxNum; i++ )
        {
			...
			if (Value > CBMax || Value < CBMin) {
				btmpresult = false;
				g_int_tptest_result |= TEST_BEYOND_MIN_LIMIT;
				tpd_test_save_procedure("Failed. Num = %d, Value = %d, range = (%d, %d):", i + 1, Value, CBMin, CBMax);
...
//       ibiggerValue = g_ScreenSetParam.iTxNum>g_ScreenSetParam.iRxNum?g_ScreenSetParam.iTxNum:g_ScreenSetParam.iRxNum;
    ...
    if (g_stCfg_FT5X46_BasicThreshold.SCapCbTest_SetWaterproof_OFF && bFlag)
    {
        ...
        if (bFlag)
            FTS_TEST_DBG("SCap CB_Rx:  ");
#ifdef CONFIG_TPD_FOCAL_TEST
		tpd_test_save_procedure("SCapCbTest_OFF_Min  Tx %d Rx %d on Min\n", g_ScreenSetParam.iTxNum, g_ScreenSetParam.iRxNum);
		tpd_save_short_array_to_file(g_stCfg_MCap_DetailThreshold.SCapCbTest_OFF_Min, 2, g_ScreenSetParam.iRxNum);
		
		tpd_test_save_procedure("SCapCbTest_OFF_Max Tx %d Rx %d on Max\n", g_ScreenSetParam.iTxNum, g_ScreenSetParam.iRxNum);
		tpd_save_short_array_to_file(g_stCfg_MCap_DetailThreshold.SCapCbTest_OFF_Max, 2, g_ScreenSetParam.iRxNum);
...
#ifdef CONFIG_TPD_FOCAL_TEST
	tpd_test_save_procedure("Panel Differ Test\n");
#endif
...
#ifdef CONFIG_TPD_FOCAL_TEST
	tpd_test_save_procedure("PanelDifferTest_Min  Tx %d Rx %d Higth Min\n", g_ScreenSetParam.iTxNum, g_ScreenSetParam.iRxNum);
	tpd_save_array_to_file(g_stCfg_MCap_DetailThreshold.PanelDifferTest_Min, g_ScreenSetParam.iTxNum, g_ScreenSetParam.iRxNum);
	
	tpd_test_save_procedure("PanelDifferTest_Max Tx %d Rx %d Higth Max\n", g_ScreenSetParam.iTxNum, g_ScreenSetParam.iRxNum);
	tpd_save_array_to_file(g_stCfg_MCap_DetailThreshold.PanelDifferTest_Max, g_ScreenSetParam.iTxNum, g_ScreenSetParam.iRxNum);
	
	tpd_test_save_procedure("RawData Tx %d Rx %d RawData\n", g_ScreenSetParam.iTxNum, g_ScreenSetParam.iRxNum);
	tpd_save_array_to_file(m_RawData, g_ScreenSetParam.iTxNum, g_ScreenSetParam.iRxNum);
	
	tpd_test_save_procedure("DifferData Tx %d Rx %d\n", g_ScreenSetParam.iTxNum, g_ScreenSetParam.iRxNum);
	tpd_save_array_to_file(m_DifferData, g_ScreenSetParam.iTxNum, g_ScreenSetParam.iRxNum);
#endif
    ...
    for (iRow = 0; iRow<g_ScreenSetParam.iTxNum; iRow++) //  iRow = 1
    {
        for (iCol = 0; iCol < g_ScreenSetParam.iRxNum; iCol++)
        {
            ...
            if (iValue < iMin || iValue > iMax)
            {
				...
				set_failed_node(iRow, iCol);
				tpd_test_save_procedure("Out Of Range.  Node=(%d %d) Get_value=%d Set_Range=(%d %d) \n", iRow, iCol, iValue, iMin, iMax);
#endif
...
unsigned char FT5X46_TestItem_WeakShortTest( bool* bTestResult )
{
...
#ifdef CONFIG_TPD_FOCAL_TEST
	tpd_test_save_procedure("Weak Short-Circuit Test\n");
#endif
...
#ifdef CONFIG_TPD_FOCAL_TEST
	tpd_test_save_procedure("Channel and Ground:%d Channel and Channel:%d\n", g_stCfg_FT5X46_BasicThreshold.WeakShortTest_CG,  g_stCfg_FT5X46_BasicThreshold.WeakShortTest_CC);
	...
	for (i = 0; i < iAllAdcDataNum /*iChannelNum */ ; i++) {
		if (i <= (iChannelNum + 1)) {
			if (i == 0) {
				tpd_test_save_procedure("\nOffset %02d: %4d \n", i, iAdcData[i]);
			} else if (i == 1) {	/*if(i <= iMaxTx) */
				tpd_test_save_procedure("Ground %02d: %4d \n", i, iAdcData[i]);
			} else if (i <= (iMaxTx + 1)) {
				tpd_test_save_procedure("Tx%02d: %4d,", i - 1, iAdcData[i]);
			} else if (i <= (iChannelNum + 1)) {
				tpd_test_save_procedure("Rx%02d: %4d,", i - iMaxTx - 1, iAdcData[i]);
			}
			...
			if (i == (iChannelNum + 2)) {
				...
				tpd_test_save_procedure("Tx%02d: %4d,", i - (iChannelNum + 2), iAdcData[i]);
			} else if (i < iAllAdcDataNum) {
				tpd_test_save_procedure("Rx%02d: %4d,", i - iMaxTx - (iChannelNum + 2), iAdcData[i]);
			}
			if (i % 10 == 0) {
				tpd_test_save_procedure("\n");
			}
    ...
    for (i = 0; i < iChannelNum; i++)
    {
		...
		if ((iMin_CG > fGShortResistance[i]) || (iDsen - iDoffset < 0))	//<=0 ShortResistance = 0
		{
			iCount++;
			if (i + 1 <= iMaxTx) {
				tpd_test_save_procedure("Tx%02d: %02d (k),", i + 1, fGShortResistance[i]);
			} else {
				tpd_test_save_procedure("Rx%02d: %02d (k),", i + 1 - iMaxTx, fGShortResistance[i]);
			}
			if (iCount % 10 == 0) {
				tpd_test_save_procedure("\n");
			}
    ...
    for ( i = 0; i < iChannelNum; i++)
    {
		...
		if (fMShortResistance[i] <= 0 || fMShortResistance[i] < iMin_CC) {
			iCount++;
			if (i + 1 <= iMaxTx) {
				tpd_test_save_procedure("Tx%02d: %02d(k),", i + 1, fMShortResistance[i]);
			} else {
				tpd_test_save_procedure("Rx%02d: %02d(k),", i + 1 - iMaxTx, fMShortResistance[i]);
			}
			if (iCount % 10 == 0) {
				tpd_test_save_procedure("\n");
			}
    ...
    {
#ifdef CONFIG_TPD_FOCAL_TEST
		g_int_tptest_result |= TEST_GT_SHORT;
#endif
        btmpresult = false;
    }
