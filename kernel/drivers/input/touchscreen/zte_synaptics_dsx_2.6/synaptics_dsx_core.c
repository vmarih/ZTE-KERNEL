...
unsigned int query_cover_mode_state = false;
...
static struct kobject *synaptics_tp_cover_mode_kobj;
...
//for cover_mode
static DEVICE_ATTR(status, (S_IRUGO | S_IWUSR | S_IWGRP),  synaptics_rmi4_cover_mode_show,  synaptics_rmi4_cover_mode_store); 

static struct attribute *synaptics_cover_mode_attributes[] = {	
	&dev_attr_status.attr,		
	NULL
};
static struct attribute_group synaptics_cover_mode_attribute_group = {	
	.attrs = synaptics_cover_mode_attributes
};
		...
		struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "%u\n",
			/*rmi4_data->enable_glove_mode*/query_cover_mode_state);
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	/*rmi4_data->enable_glove_mode*/query_cover_mode_state = input;
	return count;
}
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	unsigned char intr_data[MAX_INTR_REGISTERS + 1];
	unsigned short data_addr;
	int x;
	int y;
	int wx;
	...
	if (rmi4_data->suspend && rmi4_data->enable_wakeup_gesture) {
		...
		if ((gesture_type == 3) && gesture_type != F12_UDG_DETECT) {
			input_report_key(rmi4_data->input_dev, KEY_WAKEUP, 1);
			input_sync(rmi4_data->input_dev);
			input_report_key(rmi4_data->input_dev, KEY_WAKEUP, 0);
			input_sync(rmi4_data->input_dev);
		...
		else
		{
			retval = synaptics_rmi4_reg_read(rmi4_data,
				...
				if (retval < 0) {
					dev_err(rmi4_data->pdev->dev.parent,
								"%s: Gesture Failed to read interrupt status\n",
								__func__);
					return 0;
				}
		}
	...
	for (finger = 0; finger < fingers_to_process; finger++) {
		...
		case F12_GLOVED_FINGER_STATUS:
			...
			if (stylus_presence)
				break;
			if(query_cover_mode_state)
			{
			    if(x<70 || x >1020 || y<200 || y>1150)
                break;
			}
#ifdef TYPE_B_PROTOCOL
			input_mt_slot(rmi4_data->input_dev, finger);
		...
		unsigned int intr_count)
{
	...
	rmi4_data->stylus_enable = ctrl_23->stylus_enable;//whether to modify by zte?
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
    ...
    //end
    //	create cover mode node begin
     	synaptics_tp_cover_mode_kobj = kobject_create_and_add("tp_cover_mode", NULL);	 
		if (!synaptics_tp_cover_mode_kobj)	
		{		 
			retval = -EINVAL;		 
			printk("syna--ERROR Unable to create shtsc_cover_mode_kobj\n");	 
		}	 
		retval = sysfs_create_group(synaptics_tp_cover_mode_kobj, &synaptics_cover_mode_attribute_group);	 
		if (0 != retval)	 
		{		 
			printk("syna--ERROR: sysfs_create_cover_group() failed.\n");		  
			//sysfs_remove_group(&client->dev.kobj, &gt9xx_wake_gesture_attribute_group);		 
			kobject_put(synaptics_tp_cover_mode_kobj);		
			//return -EIO;	
		}	  
		else	 
		{		 
			printk("GTP:sysfs_create_cover_group() succeeded.\n");    
		}
    //end
//zz add by wkh for touch info
...
if(synaptics_tp_cover_mode_kobj)
	{
		sysfs_remove_group(synaptics_tp_cover_mode_kobj, &synaptics_cover_mode_attribute_group);	
		kobject_put(synaptics_tp_cover_mode_kobj);
	}
