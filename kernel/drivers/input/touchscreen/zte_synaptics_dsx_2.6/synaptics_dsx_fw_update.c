...
static ssize_t fwu_sysfs_config_id_show(struct device *dev,
		struct device_attribute *attr, char *buf);
static ssize_t fwu_sysfs_bootloader_id_show(struct device *dev,
		struct device_attribute *attr, char *buf);
//end
#ifdef SYNA_TDDI
...
//add by wkh begin
	__ATTR(config_id, S_IRUGO,
			...
			synaptics_rmi4_store_error),
	__ATTR(bootloader_id, S_IRUGO,
			fwu_sysfs_bootloader_id_show,
			synaptics_rmi4_store_error),
//end
        #ifdef SYNA_TDDI
...
int set_tddi_lockdown_data(unsigned char *lockdown_data, unsigned short leng)
{
	...
	unsigned long checksum;
	unsigned char checksum_array[4];
	unsigned short blk_cnt = fwu->blkcount.tddi_lockdown_data;
	struct synaptics_rmi4_data *rmi4_data = fwu->rmi4_data;
...
int getConfigId(char *buf)
{
	...
	dev_err(rmi4_data->pdev->dev.parent,"%s: config_id[2] is %d     config_id[3] is %d  \n",__func__,config_id[2],config_id[3] );
		...
		else if(config_id[0] == 255 && config_id[1] == 255 && config_id[2] == 255 && config_id[3] == 255)
		{
		    return snprintf(buf, 100, "SYNATP+BAD(11.11.11.11)\n");
		}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	unsigned int ld_val;//int ld_val; modify by wkh
	int i = 0;
