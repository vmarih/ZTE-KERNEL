...
int gf_parse_dts(struct gf_dev* gf_dev) {
    ...
    //power on
    gpio_direction_output(gf_dev->pwrvdd_gpio, 0);//modify by zte_sw_chenhui to PD 1p8V
...
/*added by chenhui for pwr, begin*/	
    ...
    if (gpio_is_valid(gf_dev->pwrvdd_gpio)) {
        gpio_set_value(gf_dev->pwrvdd_gpio, 0);//modify by zte_sw_chenhui to PD 1p8V
    }
...
/*added by chenhui for pwr, begin*/		
    if (gpio_is_valid(gf_dev->pwr3v3_gpio)) {
        gpio_set_value(gf_dev->pwr3v3_gpio, 0);
    }
    if (gpio_is_valid(gf_dev->pwrvdd_gpio)) {
        gpio_set_value(gf_dev->pwrvdd_gpio, 0);//modify by zte_sw_chenhui to PD 1p8V
    }	
