...
struct smb1360_chip {
	...
	u32				connected_rid;
#ifdef CONFIG_SMB1360_BATTERY_CHIP_CODE
...
static void dump_regs(struct smb1360_chip *chip);
...
static int smb1360_update_ui_soc(struct smb1360_chip *chip)
{
	...
	if((chip->fg_soc > 90) && (chip->fg_soc != chip->ui_soc) && (txc.time.tv_sec - last_time >= DELTA_TIME))
	{
		...
		{
			if(last_time != 0)	//UE improvement: do not decrease soc immediately when usb is removed. change is next time
				chip->ui_soc--;
			last_time = txc.time.tv_sec;
...
static void smb1360_jeita_work_fn(struct work_struct *work)
{
	...
	pr_debug("temp = %d, [hot warm cool cold] : [%d %d %d %d]\n",
		temp,chip->hot_bat_decidegc,chip->warm_bat_decidegc,chip->cool_bat_decidegc,chip->cold_bat_decidegc);
	...
	if (temp > chip->hot_bat_decidegc) {
		...
		rc = smb1360_set_soft_jeita_threshold(chip,
			(chip->warm_bat_decidegc - 20), chip->hot_bat_decidegc);
		if (rc) {
			dev_err(chip->dev, "Couldn't set jeita threshold\n");
			goto end;
		}
		...
		if (rc)
			pr_err("Couldn't set USB current\n");
		rc = smb1360_set_soft_jeita_threshold(chip,
			chip->cold_bat_decidegc, (chip->cool_bat_decidegc + 20));
		if (rc) {
			dev_err(chip->dev, "Couldn't set jeita threshold\n");
			goto end;
		}
...
static int hot_soft_handler(struct smb1360_chip *chip, u8 rt_stat)
{
	...
	if(chip->batt_warm){
		smb1360_set_soft_jeita_threshold(chip,
			chip->cool_bat_decidegc, (chip->warm_bat_decidegc -20));
	}else{
		smb1360_set_soft_jeita_threshold(chip,
			chip->cool_bat_decidegc, chip->warm_bat_decidegc);
	}
...
static int cold_soft_handler(struct smb1360_chip *chip, u8 rt_stat)
{
	...
	smb1360_set_recharge_thresh_fg_source(chip);
	if(chip->batt_cool){
		smb1360_masked_write(chip, CFG_FVC_REG,
			FLT_VTG_COMP_MASK, ((chip->vfloat_mv - chip->cool_bat_mv) / 10));
		smb1360_set_soft_jeita_threshold(chip,
			(chip->cool_bat_decidegc + 20), chip->warm_bat_decidegc);
	}else{
		smb1360_masked_write(chip, CFG_FVC_REG,
			FLT_VTG_COMP_MASK, ((chip->vfloat_mv - chip->warm_bat_mv) / 10));
		smb1360_set_soft_jeita_threshold(chip,
			chip->cool_bat_decidegc, chip->warm_bat_decidegc);
	}
	smb1360_set_appropriate_usb_current(chip);
...
{
	pr_info("charging error! - rt_stat = 0x%02x\n", rt_stat);
...
static int smb1360_hw_init(struct smb1360_chip *chip)
{
	...
	if (chip->client->irq) {
		...
		rc = smb1360_masked_write(chip, PRE2FAST_VOLTAGE_REG, PRE2FAST_VOLTAGE_THRESHOLD_MASK, 
		                                PRE2FAST_VOLTAGE_2V9 << PRE2FAST_VOLTAGE_THRESHOLD_SHIFT);
		if (rc < 0) {
		...
		rc = smb1360_masked_write(chip, CHG_CURRENT_REG, PRECHG_CURR_MASK, 
		                                PRECHG_CURR_550MA << PRECHG_CURR_SHIFT);
		if (rc < 0) {
...
static int smb_parse_batt_id(struct smb1360_chip *chip)
{
	...
	/* read battery ID */
#if 1
	#define PM8937_MPP2 0x21	//the channel value follow node pm8937_vadc in dtsi
	rc = qpnp_vadc_read(chip->vadc_dev, PM8937_MPP2, &result);
	if (rc) {
		pr_err("error reading batt id channel = 0x%x, rc = %d\n",
					PM8937_MPP2, rc);
		return rc;
	}
	...
	if(batt_id_uv > 1200000){
		chip->connected_rid = chip->profile_rid[0];
	}else{
		chip->connected_rid = chip->profile_rid[1];
		//of_property_read_u32(node, "qcom,fg-batt-capacity-mah_v2",
		//			&chip->batt_capacity_mah);
	}
    #ifdef CONFIG_SMB1360_BATTERY_CHIP_CODE
    if (chip->connected_rid == chip->profile_rid[0])
        chip->battery_chip_code = 20;
    else
        chip->battery_chip_code = 03;
    #endif
#else
...
static int smb1360_parse_jeita_params(struct smb1360_chip *chip)
{
	...
	if (of_property_read_bool(node, "qcom,soft-jeita-supported")) {
		...
		rc = of_property_read_u32(node, "qcom,cool-bat-mv",
						&chip->cool_bat_mv);
		if(chip->connected_rid == chip->profile_rid[1])
			rc = of_property_read_u32(node, "qcom,cool-bat-mv_v2",
						&chip->cool_bat_mv);
		if (rc) {
		...
		rc = of_property_read_u32(node, "qcom,warm-bat-mv",
						&chip->warm_bat_mv);
		if(chip->connected_rid == chip->profile_rid[1])
			rc = of_property_read_u32(node, "qcom,warm-bat-mv_v2",
						&chip->warm_bat_mv);
		if (rc) {
...
static int smb_parse_dt(struct smb1360_chip *chip)
{
	...
	rc = of_property_read_u32(node, "qcom,float-voltage-mv",
						&chip->vfloat_mv);
	if(chip->connected_rid == chip->profile_rid[1])
		rc = of_property_read_u32(node, "qcom,float-voltage-mv_v2",
						&chip->vfloat_mv);
	if (rc < 0)
	...
	rc = of_property_read_u32(node, "qcom,fg-batt-capacity-mah",
					&chip->batt_capacity_mah);
	if(chip->connected_rid == chip->profile_rid[1])
		rc = of_property_read_u32(node, "qcom,fg-batt-capacity-mah_v2",
					&chip->batt_capacity_mah);
	if (rc < 0)
	...
	rc = of_property_read_u32(node, "qcom,fg-cc-to-cv-mv",
					&chip->fg_cc_to_cv_mv);
	if(chip->connected_rid == chip->profile_rid[1])
		rc = of_property_read_u32(node, "qcom,fg-cc-to-cv-mv_v2",
					&chip->fg_cc_to_cv_mv);
	if (rc < 0)
		chip->fg_cc_to_cv_mv = -EINVAL;
	...
	pr_debug("chip->adapter_curr_limit is %d\n", chip->adapter_curr_limit);
	if (rc < 0)
