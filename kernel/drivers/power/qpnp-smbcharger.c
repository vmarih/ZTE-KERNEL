...
/*ZTE_MODIFY, add for wireless charging, start*/
#ifdef CONFIG_ZTE_WIRELESS_CHARGING
#include <linux/pinctrl/consumer.h>
#include <linux/platform_device.h>
#include <linux/delay.h>
#endif
/*ZTE_MODIFY, add for wireless charging, end*/

/*ZTE_MODIFY, add for wireless charging, start*/
#ifdef CONFIG_ZTE_WIRELESS_CHARGING
struct wireless_charging_platform_data_type
{
	int gpio_status;
	int gpio_en1;
	int gpio_en2;
	int current_enumerated;
};
struct wireless_charging_platform_data_type wireless_charging_platform_data;

void wireless_charging_disable(void);
void wireless_charging_enable(void);
#endif
/*ZTE_MODIFY, add for wireless charging, end*/
...
struct smbchg_chip {
	...
	int				usb_icl_comp;
	bool				allow_disable_hvdcp;
	//ZTE_MODIFY by DingXuehua, for jeita case, begin
	int				float_voltage_comp_warm;
	int				float_voltage_comp_cool;
	//ZTE_MODIFY by DingXuehua, for jeita case, end
...
/*ZTE DOUMM ADD ++*/
#ifdef CONFIG_ZTE_WIRELESS_CHARGING
	struct delayed_work		zte_aicl_work;
#endif
/*ZTE DOUMM ADD ++*/
	/* wipower params */
...
#endif
	//lixinghe 20170412. detect usb in charging mode
	struct delayed_work		charging_mode_detect_usb_work;

	struct delayed_work		vfloat_adjust_work;
	struct delayed_work		hvdcp_det_work;
	struct delayed_work		usb_type_recheck_work;
	struct delayed_work		capacity_compensation_work;
	struct delayed_work		cancel_all_work;
...
//ZTE-MODIFY By DingXuehua, fix sometimes machine can not power off when offcharing
struct wake_lock capacity_compensation_wakelock;
extern char *saved_command_line;
static bool charger_boot = false;
static bool is_init = false;
static bool disable_hvdcp_by_soc = false;
static bool disable_hvdcp_by_jeita = false;
static bool disable_hvdcp_by_thermal = false;
static bool disable_hvdcp = false;

extern bool g_usb_nonstandard_charger;

//ZTE-MODIFY by DingXuehua, begin
#ifdef ZTE_CHG_DEBUG
...
#endif
static int smbchg_total_aicl_ma_at_probe = 0;
...
static bool is_usb_online(struct smbchg_chip *chip)
{
	int rc;
	u8 reg;
...
/*ZTE_MODIFY by wangbin,measure system current by usb*/
...
static void smbchg_disable_hvdcp(struct smbchg_chip *chip);
#define DELTA_TIME 5
...
static int get_prop_batt_capacity(struct smbchg_chip *chip)
{
	...
	int is_charging = get_prop_batt_status(chip);
	struct timex txc;
	...
	//pr_smb(PR_INTERRUPT, "txc.time.tv_sec = %ld, last_time = %ld\n",txc.time.tv_sec,last_time);
	if(last_time > txc.time.tv_sec)
		last_time = txc.time.tv_sec;
	if((last_capacity != -1) && (txc.time.tv_sec - last_time <= DELTA_TIME))
	{
		//pr_info("capacity update too freuqently, use last capacity: %d\n",last_capacity);
	...
	if(capacity>=90){
		disable_hvdcp_by_soc = true;
	}else{
		disable_hvdcp_by_soc = false;
	}

	pr_smb(PR_INTERRUPT, "soc_now = %d, soc_last = %d, is_charging = %d\n",capacity,last_capacity,is_charging);
	if(capacity >= 0 && capacity <= 100 && last_capacity != -1){
		if((POWER_SUPPLY_STATUS_CHARGING == is_charging) && (capacity - last_capacity) > 1){
			pr_info("smooth capacity++\n");
			capacity = last_capacity + 1;
			wake_lock_timeout(&capacity_compensation_wakelock, 10 * HZ);
			cancel_delayed_work_sync(&chip->capacity_compensation_work);
			schedule_delayed_work(&chip->capacity_compensation_work,
					msecs_to_jiffies(8000));
			
		}
		if((POWER_SUPPLY_STATUS_DISCHARGING == is_charging) && (last_capacity - capacity) > 1){
			pr_info("smooth capacity--\n");
			capacity = last_capacity - 1;
			wake_lock_timeout(&capacity_compensation_wakelock, 10 * HZ);
			cancel_delayed_work_sync(&chip->capacity_compensation_work);
...
//ZTE_MODIFY by wangbin to fix the issue that some time weak charger don't charge,refer to case:02479108
#if 0
							...
							int current_ma)
{
	...
	#ifdef CONFIG_ZTE_WIRELESS_CHARGING
	if( gpio_get_value(wireless_charging_platform_data.gpio_status) == 0) {
		current_ma = 950;
		smbchg_rerun_aicl(chip);//doumm
	}	
	#endif
		...
		int total_current_ma)
{
	...
	new_parallel_cl_ma = max(total_current_ma,smbchg_total_aicl_ma_at_probe)//total_current_ma
...
/*ZTE DOUMM ADD ++*/
#ifdef CONFIG_ZTE_WIRELESS_CHARGING
static void smbchg_zte_aicl_work(struct work_struct *work)
{
	struct smbchg_chip *chip = container_of(work,
				struct smbchg_chip,
				zte_aicl_work.work);
...
static void smbchg_parallel_usb_en_work(struct work_struct *work)
{
	...
	if ((!chip->batt_warm) && (!chip->batt_cool) && (!disable_hvdcp) && smbchg_is_parallel_usb_ok(chip, &total_current_ma)){
	/** ZSW_DingXuehua, limit charge current when battery warm or cool via disable parallel charger */
...
static void smbchg_disable_hvdcp(struct smbchg_chip *chip)
{
	if(disable_hvdcp_by_soc || disable_hvdcp_by_jeita || disable_hvdcp_by_thermal)
		disable_hvdcp = true;
	else
		disable_hvdcp = false;

	pr_info("disable_hvdcp=%d, by_soc=%d, by_jeita=%d, by_thermal=%d\n",disable_hvdcp,disable_hvdcp_by_soc,disable_hvdcp_by_jeita,disable_hvdcp_by_thermal);
	if(chip->allow_disable_hvdcp && (!chip->hvdcp_not_supported)){
		smbchg_sec_masked_write(chip,
			chip->usb_chgpth_base + CHGPTH_CFG,
			HVDCP_EN_BIT, disable_hvdcp ? 0 : HVDCP_EN_BIT);
	}
}
								...
								int lvl_sel)
{
	...
	if(0 == lvl_sel && 0 != chip->therm_lvl_sel){
		pr_info("thermal limit cancel.\n");
		thermal_deactive = true;
	}
	...
	if(chip->therm_lvl_sel){
		disable_hvdcp_by_thermal = true;
	}else{
		disable_hvdcp_by_thermal = false;
	}
	smbchg_disable_hvdcp(chip);

	if(thermal_deactive && chip->usb_present){
		pr_info("thermal deactive.\n");
		cancel_delayed_work_sync(&chip->usb_type_recheck_work);
		smbchg_stay_awake(chip, PM_USB_TYPE_RECHECK);
		schedule_delayed_work(&chip->usb_type_recheck_work,
...
static int smbchg_float_voltage_set(struct smbchg_chip *chip, int vfloat_mv)
{
	...
	int rc, delta;
	u8 temp;
        //ZTE_MODIFY by wangbin,begin
	//u8 prev_trim, new_trim;
        //ZTE_MODIFY by wangbin,end
...
static void smbchg_usb_type_recheck_work(struct work_struct *work)
{
	struct smbchg_chip *chip = container_of(work,
				struct smbchg_chip,
				usb_type_recheck_work.work);
	enum power_supply_type usb_supply_type;
	...
	if(chip->usb_present)
	{
		...
		if(usb_supply_type != POWER_SUPPLY_TYPE_UNKNOWN){
			...
			msleep(100);
			aicl_ma = smbchg_get_aicl_level_ma(chip);
			pr_smb(PR_MISC, "aicl_ma = %d\n",aicl_ma);
			rc = vote(chip->usb_icl_votable, PSY_ICL_VOTER, true, aicl_ma);
		}
		...
		if(usb_supply_type == POWER_SUPPLY_TYPE_USB_DCP){
			...
			if(is_hvdcp_present(chip)){
				...
				if (!chip->hvdcp_not_supported) {
					...
					schedule_delayed_work(&chip->hvdcp_det_work, msecs_to_jiffies(2500));
				}
	...
	pr_info("exit.\n");
	g_usb_nonstandard_charger = false;
	//dump_regs(chip);
	smbchg_relax(chip, PM_USB_TYPE_RECHECK);
}

static void smbchg_capacity_compensation_work(struct work_struct *work)
{
	struct smbchg_chip *chip = container_of(work,
				struct smbchg_chip,
				capacity_compensation_work.work);

	if (chip->psy_registered)
		power_supply_changed(&chip->batt_psy);
}

static void smbchg_cancel_all_work(struct work_struct *work)
{
	struct smbchg_chip *chip = container_of(work,
				struct smbchg_chip,
				cancel_all_work.work);

	if(!is_usb_present(chip)){
		cancel_delayed_work_sync(&chip->parallel_en_work);
		cancel_delayed_work_sync(&chip->vfloat_adjust_work);
		cancel_delayed_work_sync(&chip->hvdcp_det_work);
		pr_info("chip->wake_reasons = %d\n",chip->wake_reasons);
		smbchg_relax(chip,chip->wake_reasons);
	}
}
...
static void smbchg_external_power_changed(struct power_supply *psy)
{
	...
	/*ZTE_MODIFY, add for wireless charging, start*/
//	#ifdef CONFIG_ZTE_WIRELESS_CHARGING
//	wireless_charging_platform_data.current_enumerated = current_limit;
//	if( gpio_get_value(wireless_charging_platform_data.gpio_status) == 0){
//		printk("smbchg_set_usb_current_max doumm gpio_status =0 \n");
//		current_limit = 900;
//	}else
//		current_limit = wireless_charging_platform_data.current_enumerated;
//	#endif
	/*ZTE_MODIFY, add for wireless charging, end*/
...
static irqreturn_t batt_warm_handler(int irq, void *_chip)
{
	...
	if(chip->batt_warm){
		disable_hvdcp_by_jeita = true;
	}else{
		disable_hvdcp_by_jeita = false;
	}
	smbchg_disable_hvdcp(chip);
...
static irqreturn_t src_detect_handler(int irq, void *_chip)
{
	...
	if(src_detect && probe_done_flag)
		wake_lock_timeout(&usbin_valid_irq_wakelock, 10* HZ);	//unlock after 5*HZ
	...
	if((!usb_present) && (!chip->hvdcp_not_supported)){
		pr_info("chip->wake_reasons = %d\n",chip->wake_reasons);
		schedule_delayed_work(&chip->cancel_all_work, msecs_to_jiffies(5000));
	}
...
static irqreturn_t usbid_change_handler(int irq, void *_chip)
{
	...
	/*ZTE_MODIFY, add for wireless charging, start*/
	#ifdef CONFIG_ZTE_WIRELESS_CHARGING_OTG
	if(otg_present) {
		wireless_charging_disable();
	} else {
		wireless_charging_enable();
	}
	#endif
	/*ZTE_MODIFY, add for wireless charging, end*/
...
static int smbchg_hw_init(struct smbchg_chip *chip)
{
	...
	//ZTE_MODIFY by DingXuehua, disable charger safety timer, begin
	if(chip->safety_timer_en == true)
		smbchg_safety_timer_enable(chip, false);
	//ZTE_MODIFY by DingXuehua, disable charger safety timer, end
...
static int smb_parse_dt(struct smbchg_chip *chip)
{
	...
	chip->allow_disable_hvdcp = of_property_read_bool(node,
					"qcom,allow-disable-hvdcp");
	pr_info("allow_disable_hvdcp=%d\n",chip->allow_disable_hvdcp);
	//ZTE_MODIFY by DingXuehua, for jeita case, begin
...
//lixinghe 20170412. detect usb in charging mode start
static void smbchg_charging_mode_detect_usb_work(struct work_struct *work)
{
	#define DELAY_SEC 3000
	struct smbchg_chip *chip = container_of(work,
				struct smbchg_chip,
				charging_mode_detect_usb_work.work);
	bool usb_present=true;
	static bool last_usb_present =true;
	usb_present = is_usb_online(chip);

	pr_smb(PR_STATUS, "usb_present:%d\n", usb_present);
	
	if(last_usb_present == false && usb_present == false)
	{
		pr_smb(PR_STATUS, "usb out for 3 second machine_power_off\n");
		machine_power_off();
	}

	last_usb_present = usb_present;
	schedule_delayed_work(&chip->charging_mode_detect_usb_work,
			msecs_to_jiffies(DELAY_SEC));
	
}
//lixinghe 20170412. detect usb in charging mode end
static int smbchg_probe(struct spmi_device *spmi)
{
	...
	INIT_DELAYED_WORK(&chip->usb_type_recheck_work, smbchg_usb_type_recheck_work);
	INIT_DELAYED_WORK(&chip->capacity_compensation_work, smbchg_capacity_compensation_work);
	INIT_DELAYED_WORK(&chip->cancel_all_work, smbchg_cancel_all_work);
	...
	if(smbchg_parallel_en){
		if(chip->usb_present)
			smbchg_total_aicl_ma_at_probe = smbchg_get_aicl_level_ma(chip);
		else
			smbchg_total_aicl_ma_at_probe = smbchg_default_dcp_icl_ma;
		pr_info("smbchg_total_aicl_ma_at_probe = %d\n",smbchg_total_aicl_ma_at_probe);
	}
	...
	wake_lock_init(&capacity_compensation_wakelock, WAKE_LOCK_SUSPEND, "cap_comp_wl");
	probe_done_flag = true;
	
/*ZTE DOUMM ADD ++*/
#ifdef CONFIG_ZTE_WIRELESS_CHARGING
	INIT_DELAYED_WORK(&chip->zte_aicl_work, smbchg_zte_aicl_work);
	if(chip->usb_present) {
		printk("doumm usb_present zte aicl work\n");
		schedule_delayed_work(&chip->zte_aicl_work, 2*HZ);
	}
#endif
/*ZTE DOUMM ADD ++*/
	...
	{	
		INIT_DELAYED_WORK(&chip->charging_mode_detect_usb_work, smbchg_charging_mode_detect_usb_work);
		schedule_delayed_work(&chip->charging_mode_detect_usb_work,0);	
	}		
	//lixinghe 20170412. detect usb in charging mode end	
...
static int smbchg_remove(struct spmi_device *spmi)
{
	...
	wake_lock_destroy(&capacity_compensation_wakelock);
...
static void smbchg_shutdown(struct spmi_device *spmi)
{
	...
	if(start_flag == true)
	{
		if(wait_for_completion_timeout(&chip->driver_shutdown_completion, msecs_to_jiffies(10000)) == 0)
...
#ifdef CONFIG_ZTE_WIRELESS_CHARGING
#ifdef CONFIG_ZTE_WIRELESS_CHARGING_OTG
static int zte_wireless_charging_disabled = 0;
static DEFINE_SPINLOCK(wireless_charging_lock);
#endif
static struct delayed_work charging_switch_check_work;
static void charging_switch_check(struct work_struct *work)
{
	static int wireless_charging_status = 0;
	int current_status;
	if( gpio_get_value(wireless_charging_platform_data.gpio_status) == 0)
		current_status = 1;
	else
		current_status = 0;
printk("wireless:charging_switch_check  current_status=%d, wireless_charging_status=%d\n",current_status,wireless_charging_status);
	if(current_status != wireless_charging_status) {
		if(wireless_charging_status) {
			#ifdef CONFIG_ZTE_WIRELESS_CHARGING_OTG
			spin_lock(&wireless_charging_lock);
			if(zte_wireless_charging_disabled == 0) {
			#endif
				gpio_set_value(wireless_charging_platform_data.gpio_en1, 1);
				gpio_set_value(wireless_charging_platform_data.gpio_en2, 1);
			#ifdef CONFIG_ZTE_WIRELESS_CHARGING_OTG
			}
			spin_unlock(&wireless_charging_lock);
			#endif
			msleep(1000);
			#ifdef CONFIG_ZTE_WIRELESS_CHARGING_OTG
			spin_lock(&wireless_charging_lock);
			if(zte_wireless_charging_disabled == 0) {
			#endif
				gpio_set_value(wireless_charging_platform_data.gpio_en1, 0);
				gpio_set_value(wireless_charging_platform_data.gpio_en2, 0);
			#ifdef CONFIG_ZTE_WIRELESS_CHARGING_OTG
			}
			spin_unlock(&wireless_charging_lock);
			#endif
		}
		wireless_charging_status = current_status;
	}
	schedule_delayed_work(&charging_switch_check_work, HZ);
}
...
{
	spin_lock(&wireless_charging_lock);
	zte_wireless_charging_disabled = 1;
	gpio_set_value(wireless_charging_platform_data.gpio_en1, 1);
	gpio_set_value(wireless_charging_platform_data.gpio_en2, 0);
	spin_unlock(&wireless_charging_lock);
}
...
void wireless_charging_enable(void)
{
	...
	gpio_set_value(wireless_charging_platform_data.gpio_en1, 0);
	gpio_set_value(wireless_charging_platform_data.gpio_en2, 0);
	zte_wireless_charging_disabled = 0;
	spin_unlock(&wireless_charging_lock);
}
#endif

static int wireless_charging_probe(struct platform_device *pdev)
{
	...
	ret = of_get_named_gpio(pdev->dev.of_node, "wireless-charging-gpios", 0);
	if(gpio_is_valid(ret))
					wireless_charging_platform_data.gpio_status = ret;
	else 
		return -EINVAL;
	ret = of_get_named_gpio(pdev->dev.of_node, "wireless-charging-gpios", 1);
	if(gpio_is_valid(ret))
					wireless_charging_platform_data.gpio_en1= ret;
	else 
		return -EINVAL;
	ret = of_get_named_gpio(pdev->dev.of_node, "wireless-charging-gpios", 2);
	if(gpio_is_valid(ret))
					wireless_charging_platform_data.gpio_en2= ret;
	else 
		return -EINVAL;

	printk("wireless_charging: status %d, en1 %d, en2 %d.\n", wireless_charging_platform_data.gpio_status, \
				wireless_charging_platform_data.gpio_en1, wireless_charging_platform_data.gpio_en2);
	
	ret = gpio_request(wireless_charging_platform_data.gpio_status, "wireless_status");
	if(ret) {
		pr_err("wireless_status %d request failed.\n", wireless_charging_platform_data.gpio_status);
		return -EBUSY;
	}
	ret = gpio_request(wireless_charging_platform_data.gpio_en1, "wireless_en0");
	if(ret) {
		pr_err("wireless en1 %d  request failed.\n", wireless_charging_platform_data.gpio_en1);
		return -EBUSY;
	}
	ret = gpio_request(wireless_charging_platform_data.gpio_en2, "wireless_en1");
	if(ret) {
		pr_err("wireless en2 %d  request failed.\n", wireless_charging_platform_data.gpio_en2);
		return -EBUSY;
	}
	
	gpio_direction_input(wireless_charging_platform_data.gpio_status);
	gpio_direction_output(wireless_charging_platform_data.gpio_en1, 0);
	gpio_direction_output(wireless_charging_platform_data.gpio_en2, 0);
	gpio_set_value(wireless_charging_platform_data.gpio_en1, 0);
	gpio_set_value(wireless_charging_platform_data.gpio_en2, 0);

	INIT_DELAYED_WORK(&charging_switch_check_work, charging_switch_check);
	schedule_delayed_work(&charging_switch_check_work, HZ);

	return 0;	
	
}
static int wireless_charging_remove(struct platform_device *pdev)
{
	gpio_free(wireless_charging_platform_data.gpio_status);
	gpio_free(wireless_charging_platform_data.gpio_en1);
	gpio_free(wireless_charging_platform_data.gpio_en2);

	cancel_delayed_work_sync(&charging_switch_check_work);
	return 0;
}


static struct of_device_id wireless_charging_match_table[] = {
	{ .compatible = "zte,wireless-charging",},
	{ },
};

static struct platform_driver wireless_charging_driver = {
	.probe = wireless_charging_probe,
	.remove = wireless_charging_remove,
	.driver = {
		.owner = THIS_MODULE,
		.name = "zte-wireless-charging",
		.of_match_table = wireless_charging_match_table,
	},
};

static int __init zte_wireless_charging_init(void)
{
	return platform_driver_register(&wireless_charging_driver);
}

static void __exit zte_wireless_charging_exit(void)
{
	platform_driver_unregister(&wireless_charging_driver);
}

module_init(zte_wireless_charging_init);
module_exit(zte_wireless_charging_exit);
#endif
/*ZTE_MODIFY, add for wireless charging, end*/
