...
#include <linux/proc_fs.h> //added by zte20170118
...
struct smb358_charger {
	...
	struct delayed_work battery_temp_work;    //added by zte20170207
};
...
static int smb358_get_prop_batt_current_now(struct smb358_charger *chip)
{
	...
	if (rc) {
		printk("Couldn't get current rc = %d\n", rc);
...
static int smb358_get_prop_battery_voltage_now(struct smb358_charger *chip)
{
	...
	if (rc) {
		printk("Couldn't get voltage rc = %d\n", rc);
...
static int smb358_get_prop_batt_capacity(struct smb358_charger *chip)
{
	...
	//uv = smb358_get_prop_battery_voltage_now(chip);
	//if(uv < 3500) {
	//	soc = 1;
	//}
...
static int smb358_get_prop_batt_health(struct smb358_charger *chip)
{
	...
	if(POWER_SUPPLY_HEALTH_OVERVOLTAGE == chip->psy_health_sts)
		...
		int current_ma)
{
	...
	dev_notice(chip->dev, "%s: USB current_ma = %d\n", __func__, current_ma);
...
static void smb358_external_power_changed(struct power_supply *psy)
{
	...
	dev_notice(chip->dev, "%s set current_limit = %d\n", __func__, current_limit);
}
	...
	char __user *buffer, size_t count, loff_t *offset)
{
	...
	return simple_read_from_buffer(buffer, count, offset, g_smb_regs_buffer, g_ilength);
}
...
static struct file_operations smb_proc_ops = {
	.open = simple_open,
...
{
	smb_proc_entry = proc_create(TOUCH_PROC_FILE, 0666, NULL, &smb_proc_ops);
	if (smb_proc_entry) {
		//smb_proc_entry->data = (void*)data;
		g_chip = data;
...
static void dump_regs(struct smb358_charger *chip)
{
	...
	for (addr = 0; addr <= LAST_CNFG_REG; addr++) {
		...
		g_ilength += snprintf(g_smb_regs_buffer + g_ilength, SMB_REGS_LENGTH - g_ilength, "0x%02x = 0x%02x\n", addr, reg);
	}
...
static void smb358_battery_temp_work(struct work_struct *work)
{
	int batt_temp;
	bool bat_hot = 0, bat_cold = 0, bat_present = 0, 
		bat_warm = 0,bat_cool = 0;
	int  soc = 0, current_now = 0, voltage_now = 0, chg_type = 0;
	struct smb358_charger *chip = container_of(work,
				struct smb358_charger,
				battery_temp_work.work);
	...
	dev_notice(chip->dev, "%s batt_temp = %d\n", __func__, batt_temp);
	...
	dev_notice(chip->dev,"hot %d, cold %d, warm %d, cool %d, jeita supported %d, missing %d\n",
		chip->batt_hot, chip->batt_cold, chip->batt_warm,
		chip->batt_cool, chip->jeita_supported, chip->battery_missing);
	dev_notice(chip->dev,"batterydata:bat_vol=\t%d\t,Ibat=\t%d\t,T=\t%d\t,SOC=\t%d\t,CHR_Type=\t%d\t\n",voltage_now/1000, current_now/1000, batt_temp, soc,chg_type);
	schedule_delayed_work(&chip->battery_temp_work, msecs_to_jiffies(10000));
...
static void smbchg_battery_monitor_work(struct work_struct *work)
{
	...
	schedule_delayed_work(&chip->battery_monitor_work,msecs_to_jiffies(20000));
}
#endif
				...
				const struct i2c_device_id *id)
{
	...
	//rc = smb358_read_reg(chip, CHG_OTH_CURRENT_CTRL_REG, &reg);
	//if(rc) {
	//	for(i_address = 0x3; i_address < 0x70; i_address++) {
	//		/* probe the device to check if its actually connected */
	//		//printk("%s i2c_address:0x%x", __func__, i_address);
	//		chip->client->addr = i_address;
	//		rc = smb358_read_reg(chip, 0xe, &reg);
	//		if (rc) {
	//			pr_err("Failed to detect SMB358, device absent, rc = %d\n", rc);
	//		}
	//		printk("%s i2c_address:0x%x, val:0x%x\n", __func__, i_address, reg);
	//		//if((reg >> 1) == i_address) {    // || i_address == 0xe) {
	//		//	break;
	//		//}
	//	}
	//	if(i_address >= 0x70) {
	...
	INIT_DELAYED_WORK(&chip->battery_temp_work, smb358_battery_temp_work);    //added by zte20170118
	schedule_delayed_work(&chip->battery_temp_work,msecs_to_jiffies(5000));
