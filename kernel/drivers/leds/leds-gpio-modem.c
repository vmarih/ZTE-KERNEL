		...
		enum led_brightness value)
{
	...
	/*ZTE_MODIFY, control led in poweroff-charger mode, start*/
	...
	/*ZTE_MODIFY, control led in poweroff-charger mode, end*/
...
static ssize_t store_front_flashlight(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
 ...
 printk("front flashlight begin");
   
     if(buf != NULL && size != 0)
    {
        my_val = simple_strtoul(buf,&pvalue,2);
	gpio_direction_output(45,my_val);	
		
}
	  printk("front flashlight end value =%d",my_val);
	 return size;
}
