				...
				struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	const void *p;
	/*ZSW_MODIFY begin,eeprom who used same ic use one kernel to save probe time, jiangwanquan, 20170405*/
	char eeprom_name_vendor[32]={0};
	/*ZSW_MODIFY end*/
	...
	/*ZSW_MODIFY begin,eeprom who used same ic use one kernel to save probe time, jiangwanquan, 20170405*/
	strncpy(eeprom_name_vendor,s_ctrl->sensordata->eeprom_name,eeprom_name_len);
	CDBG("%s:%d Eeprom name is %s\n",
			__func__, __LINE__, eeprom_name_vendor);
	if(!strcmp(eeprom_name_vendor, "zte_ov13855")){
		memset(eeprom_name_vendor,0x0,sizeof(eeprom_name_vendor));
		strcpy(eeprom_name_vendor,"zte_gt24c64a");
	}else if(!strcmp(eeprom_name_vendor, "zte_s5k3l8")){
	    memset(eeprom_name_vendor,0x0,sizeof(eeprom_name_vendor));
		strcpy(eeprom_name_vendor,"zte_imx258");
	}
	/*ZSW_MODIFY end*/
	...
	*eeprom_subdev_id = -1;
	...
	for (i = 0; i < count; i++) {
		...
		#else
		if (!userspace_probe &&
			strcmp(eeprom_name, eeprom_name_vendor))
			continue;
		#endif
		/*ZSW_MODIFY end*/
