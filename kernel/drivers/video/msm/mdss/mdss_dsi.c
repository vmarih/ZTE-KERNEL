	...
	struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	ctrl_pdata->disp_vddio_en_gpio = of_get_named_gpio(ctrl_pdev->dev.of_node,"qcom,disp_vddio_en_gpio", 0);

        if (!gpio_is_valid(ctrl_pdata->disp_vddio_en_gpio)) {
                pr_err("%s:%d, qcom,disp_vddio_en_gpio not specified\n", __func__, __LINE__);
        }
        else{
                if (gpio_request(ctrl_pdata->disp_vddio_en_gpio,"disp_vddio_en_gpio")) {
                        pr_err("request disp_vddio_en_gpio failed\n");
                }else{
                    gpio_direction_output(ctrl_pdata->disp_vddio_en_gpio, 1);//config gpio to output
                    pr_err("%s:request disp_vddio_en_gpio sucess\n",__func__);
                }
        }

	ctrl_pdata->disp_vddio_n_en_gpio = of_get_named_gpio(ctrl_pdev->dev.of_node,"qcom,disp_vddio_n_en_gpio", 0);

        if (!gpio_is_valid(ctrl_pdata->disp_vddio_n_en_gpio)) {
                pr_err("%s:%d, qcom,disp_vddio_n_en_gpio not specified\n", __func__, __LINE__);
        }
        else{
                if (gpio_request(ctrl_pdata->disp_vddio_n_en_gpio,"disp_vddio_n_en_gpio")) {
                        pr_err("request disp_vddio_n_en_gpio failed\n");
                }else{
                    gpio_direction_output(ctrl_pdata->disp_vddio_n_en_gpio, 1);//config gpio to output
                    pr_err("%s:request disp_vddio_n_en_gpio sucess\n",__func__);
                }
        }
