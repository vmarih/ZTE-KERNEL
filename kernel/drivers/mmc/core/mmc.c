...
static int ddr_id_proc_show(struct seq_file *m, void *v)
{
 ...
 if(card->ext_csd.sectors <16000000) 
			 ...
			 seq_printf(m, 
		          "%s-NA-NA-2GB-LPDDR3\n",
		          manfname);
else if( card->ext_csd.sectors <64000000) 
			 seq_printf(m, 
		          "%s-NA-NA-3GB-LPDDR3\n",
		          manfname);
else
...
static struct attribute *mmc_std_attrs[] = {
	...
	&dev_attr_info.attr,// [ECID:000000] ZTEBSP wangxiaomei record eMMC info into /proc/driver/emmc file, 20131108, end
...
int mmc_attach_mmc(struct mmc_host *host)
{
	int err;
	u32 ocr, rocr;
       static bool emmc_proc_init = false; // [ECID:000000] ZTEBSP wangxiaomei record eMMC info into /proc/driver/emmc file, 20131108
