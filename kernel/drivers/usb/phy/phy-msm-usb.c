...
bool g_usb_nonstandard_charger = false;
...
phcd_retry:
	...
	//enable_irq(motg->async_irq);
	 if (host_bus_suspend || device_bus_suspend) {   // patch: for msm_otg wakelock
	     /* Enable ASYNC IRQ during LPM */ 
	     enable_irq(motg->async_irq); 
	     enable_irq(motg->irq); 
	} 
	...
	//enable_irq(motg->irq);// patch: for msm_otg wakelock
...
static int msm_otg_resume(struct msm_otg *motg)
{
    ...
    if (motg->host_bus_suspend || motg->device_bus_suspend) // patch: for msm_otg wakelock
	disable_irq(motg->irq);
...
static void msm_chg_check_dcd_flchg(struct msm_otg *motg)
{
	...
	if (!check_dcd) {
		if (floated_chg == FLOATING_AS_DCP){
			motg->chg_type = USB_DCP_CHARGER;
			printk("g_usb_nonstandard_charger\n");
			g_usb_nonstandard_charger = true;
		}
...
//					schedule_delayed_work(&motg->invalid_chg_work, 3*HZ);//ZTE_MODIFY by wangbin 5=>3 for usb notify power supply update usbonline status in time
					/*end*/
//zz 4
